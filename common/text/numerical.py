...
import inflect
import re
_magnitudes = ['триллион', 'миллиард', 'миллион', 'тысяч', 'сотни', 'мил', 'тыс']
_magnitudes_key = {'тыс': 'тысяч', 'мил': 'миллион'}
_measurements = '(f|c|k|d|m)'
_measurements_key = {'f': 'fahrenheit',
                     'c': 'celsius',
                     'k': 'thousand',
                     'm': 'meters'}
_currency_key = {'$': 'долларов', '£': 'фунтов', '€': 'евро'}
_inflect = inflect.engine()
_comma_number_re = re.compile(r'([0-9][0-9\,]+[0-9])')
_decimal_number_re = re.compile(r'([0-9]+\.[0-9]+)')
_currency_re = re.compile(r'([\$€£₩])([0-9\.\,]*[0-9]+)(?:[ ]?({})(?=[^а-яА-Я]|$))?'.format("|".join(_magnitudes)), re.IGNORECASE)
...
def _expand_currency(m):
    ...
    if dollars and cents:
        ...
        cent_unit = 'цент' if cents == 1 else 'центов'
        return "{} {}, {} {}".format(
...
def _expand_number(m):
    ...
    if number > 1000 < 10000 and (number % 100 == 0) and (number % 1000 != 0):
        text = _inflect.number_to_words(number // 100) + " ста"
    elif number > 1000 and number < 3000:
        if number == 2000:
            text = 'две тысячи'
        elif number > 2000 and number < 2010:
            text = 'две тысячи ' + _inflect.number_to_words(number % 100)
        elif number % 100 == 0:
            text = _inflect.number_to_words(number // 100) + 'тысяч'
        else:
            number = _inflect.number_to_words(number, andword='', zero='ноль', group=2).replace(', ', ' ')
