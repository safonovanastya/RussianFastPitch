...
def parse_args(parser):
    ...
    transf.add_argument('--pitch-transform-custom', type=int, default='0',
                        help='Use from 1 to 6 to set IK by Bryzgunova and ++ to set the speech accent')

    txt = parser.add_argument_group('Text processing parameters')
    txt.add_argument('--text-cleaners', type=str, nargs='*',
                     default=['russian_cleaner'],
...
def build_pitch_transformation(args):
    if args.pitch_transform_custom:
        ik_number = args.pitch_transform_custom
        def custom_(pitch, pitch_lens, mean, std):
            fields = load_fields(args.input)
            return (pitch_transform_custom(pitch * std + mean, pitch_lens, fields['text'], ik_number)
                    - mean) / std
        return custom_
